...
function wcgltToUsd(amount: number): number {
...
async function fetchTxsForAddress(
  ...
  const data = await res.json() as { status: string; result?: BscTx[] };
  if (data.status !== '1' || !Array.isArray(data.result)) return [];
  return data.result;
}

/* ── Process one token across all user addresses ────────────────────────── */
async function checkDepositsForToken(
  ...
  for (const { user_id, bsc_address } of addresses) {
    ...
    for (const tx of txs) {
      ...
      if (tx.to.toLowerCase() !== bsc_address.toLowerCase()) continue;
      ...
      const amountUsd =
        symbol === 'USDT' ? amount : wcgltToUsd(amount);
...
/* ── Start polling loop ─────────────────────────────────────────────────── */
export function startBscPoller(
  supabase: SupabaseClient,
    ...
    for (const token of SUPPORTED_TOKENS) {
      try {
        await checkDepositsForToken(
          ...
          token.decimals,
          apiKey,
          (msg, data) => data ? logger.info(msg, data) : logger.info(msg),
        );
      } catch (err) {
