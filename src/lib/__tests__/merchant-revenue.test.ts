...
describe('Per-merchant revenue calculation', () => {
  it('single merchant, single transaction', () => {
    const txs: RawTx[] = [
      { merchant_id: 'm1', amount: 500, fee: 20, net_amount: 480 },
    ];
    ...
    assert.equal(result[0].client_fees, 20);       // 4% of 500 = 20
    assert.equal(result[0].avada_cost, 15);         // 3% of 500 = 15
    assert.equal(result[0].net_margin, 5);          // 1% of 500 = 5
    assert.equal(result[0].net_amount_owed, 480);   // 500 - 20 = 480
  });

  it('multiple merchants, multiple transactions', () => {
    const txs: RawTx[] = [
      { merchant_id: 'm1', amount: 1000, fee: 40, net_amount: 960 },
      { merchant_id: 'm1', amount: 2000, fee: 80, net_amount: 1920 },
      { merchant_id: 'm2', amount: 500, fee: 20, net_amount: 480 },
    ];
    ...
    assert.equal(shopA.client_fees, 120);      // 40 + 80
    assert.equal(shopA.avada_cost, 90);        // 3000 × 0.03
    assert.equal(shopA.net_margin, 30);        // 3000 × 0.01
    assert.equal(shopA.net_amount_owed, 2880); // 960 + 1920

    const shopB = result.find((r) => r.merchant_id === 'm2')!;
    assert.equal(shopB.transaction_count, 1);
    assert.equal(shopB.volume_collected, 500);
    assert.equal(shopB.net_margin, 5);
  });

  it('margin is always 1% of volume (not of fee)', () => {
    const txs: RawTx[] = [
      { merchant_id: 'm1', amount: 10000, fee: 400, net_amount: 9600 },
    ];
    const result = computeRevenue(txs, new Map([['m1', 'Test']]));
    // net_margin = amount × 0.01 = 100, NOT fee × 0.25 = 100
    assert.equal(result[0].net_margin, 100);
    assert.equal(result[0].avada_cost, 300);  // 10000 × 0.03
    assert.equal(result[0].client_fees, 400); // stored fee
  });

  it('client_fees uses stored fee, not recalculated 4%', () => {
    // If the stored fee differs from 4% (e.g. different rate was applied),
    // the route uses the stored value for client_fees
    const txs: RawTx[] = [
      { merchant_id: 'm1', amount: 1000, fee: 50, net_amount: 950 }, // 5% fee
    ];
    const result = computeRevenue(txs, new Map([['m1', 'Test']]));
    assert.equal(result[0].client_fees, 50);  // uses stored fee
    assert.equal(result[0].avada_cost, 30);   // 3% of amount (constant)
    assert.equal(result[0].net_margin, 10);   // 1% of amount (constant)
  });
  ...
  it('totals aggregation across all merchants', () => {
    ...
    assert.equal(totals.client_fees, 60);
    assert.equal(totals.avada_cost, 45);
    assert.equal(totals.net_margin, 15);
    assert.equal(totals.net_amount_owed, 1440);
  });

  it('sorting by margin descending', () => {
    const txs: RawTx[] = [
      { merchant_id: 'm1', amount: 100, fee: 4, net_amount: 96 },
      { merchant_id: 'm2', amount: 10000, fee: 400, net_amount: 9600 },
      { merchant_id: 'm3', amount: 1000, fee: 40, net_amount: 960 },
    ];
    const result = computeRevenue(txs, new Map([['m1', 'A'], ['m2', 'B'], ['m3', 'C']]));
    result.sort((a, b) => b.net_margin - a.net_margin);

    assert.equal(result[0].merchant_id, 'm2'); // highest margin (100)
    assert.equal(result[1].merchant_id, 'm3'); // medium margin (10)
    assert.equal(result[2].merchant_id, 'm1'); // lowest margin (1)
  });
  ...
  it('sorting by transaction count descending', () => {
    const txs: RawTx[] = [
      ...
      { merchant_id: 'm1', amount: 100, fee: 4, net_amount: 96 },
      { merchant_id: 'm1', amount: 100, fee: 4, net_amount: 96 },
      { merchant_id: 'm2', amount: 50000, fee: 2000, net_amount: 48000 },
    ];
    const result = computeRevenue(txs, new Map([['m1', 'A'], ['m2', 'B']]));
  ...
  it('consistency: net_margin = client_fees - avada_cost when fee = 4%', () => {
    // When the stored fee is exactly 4% (the normal case):
    // client_fees = amount × 0.04
    // avada_cost = amount × 0.03
    // net_margin = amount × 0.01 = client_fees - avada_cost
    const txs: RawTx[] = [
      { merchant_id: 'm1', amount: 1000, fee: 40, net_amount: 960 },
    ];
    const result = computeRevenue(txs, new Map([['m1', 'Test']]));
  ...
  it('consistency: net_amount_owed = volume - client_fees (when fee = 4%)', () => {
    const txs: RawTx[] = [
      { merchant_id: 'm1', amount: 1000, fee: 40, net_amount: 960 },
    ];
    const result = computeRevenue(txs, new Map([['m1', 'Test']]));
  ...
  it('CSV export format', () => {
    ...
    assert.ok(csvLine.includes('5'));  // net_margin
  });
});
