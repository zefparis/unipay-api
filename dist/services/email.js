...
exports.sendWalletWelcomeEmail = sendWalletWelcomeEmail;
exports.sendWalletDepositEmail = sendWalletDepositEmail;
exports.sendWalletTransferEmail = sendWalletTransferEmail;
exports.sendWalletWithdrawalEmail = sendWalletWithdrawalEmail;
exports.sendWalletPinChangedEmail = sendWalletPinChangedEmail;
exports.sendAdminNewMerchantEmail = sendAdminNewMerchantEmail;
exports.sendGasAlertEmail = sendGasAlertEmail;
const brevo_1 = require("@getbrevo/brevo");
const env_js_1 = require("../config/env.js");
/* ── singleton API client ───────────────────────────────────── */
function getClient() {
    if (!env_js_1.env.BREVO_API_KEY)
        return null;
    return new brevo_1.BrevoClient({ apiKey: env_js_1.env.BREVO_API_KEY });
}
/* ── shared layout ──────────────────────────────────────────── */
...
/* ── sendWelcomeEmail ───────────────────────────────────────── */
async function sendWelcomeEmail(to, name, apiKey) {
    const api = getClient();
    if (!api) {
  ...
  `;
    await api.transactionalEmails.sendTransacEmail({
        ...
        sender: { name: env_js_1.env.BREVO_SENDER_NAME, email: env_js_1.env.BREVO_SENDER_EMAIL },
        to: [{ email: to, name }],
    });
}
...
function amountBox(sign, amount, currency, label, color, bg, border) {
    return `
...
async function sendWalletEmail(to, name, subject, bodyHtml, lang) {
    const api = getClient();
    if (!api)
        return;
    const html = layout(`
...
/* ── Wallet welcome ─────────────────────────────────────────── */
async function sendWalletWelcomeEmail(to, name, phone, lang = 'fr') {
    const fr = lang !== 'en';
    const body = `
...
/* ── Deposit confirmed ──────────────────────────────────────── */
async function sendWalletDepositEmail(params) {
    ...
    await sendWalletEmail(params.to, params.name, fr ? `Dépôt confirmé — ${params.amount} ${params.currency}` : `Deposit confirmed — ${params.amount} ${params.currency}`, body, params.lang ?? 'fr');
}
/* ── Transfer sent ──────────────────────────────────────────── */
async function sendWalletTransferEmail(params) {
    const fr = params.lang !== 'en';
    const date = new Date().toLocaleString(fr ? 'fr-FR' : 'en-GB');
    const body = `
    ...
    await sendWalletEmail(params.to, params.name, fr ? `Transfert envoyé — ${params.amount} ${params.currency}` : `Transfer sent — ${params.amount} ${params.currency}`, body, params.lang ?? 'fr');
}
/* ── Withdrawal initiated ───────────────────────────────────── */
async function sendWalletWithdrawalEmail(params) {
    const fr = params.lang !== 'en';
    const body = `
    ...
    await sendWalletEmail(params.to, params.name, fr ? `Retrait effectué — ${params.amount} ${params.currency}` : `Withdrawal completed — ${params.amount} ${params.currency}`, body, params.lang ?? 'fr');
}
/* ── PIN changed ────────────────────────────────────────────── */
async function sendWalletPinChangedEmail(params) {
    const fr = params.lang !== 'en';
    const body = `
    ...
    await sendWalletEmail(params.to, params.name, fr ? 'Votre PIN a été modifié — UniPay Congo' : 'Your PIN has been changed — UniPay Congo', body, params.lang ?? 'fr');
}
/* ── sendAdminNewMerchantEmail ──────────────────────────────── */
async function sendAdminNewMerchantEmail(merchantName, merchantEmail, company) {
    const api = getClient();
    if (!api)
        return;
    const body = `
...
async function sendGasAlertEmail(walletAddress, currentBnb, threshold, adminEmails) {
    const api = getClient();
    if (!api) {
