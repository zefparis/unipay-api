...
const adminWalletRoute = async (fastify) => {
    ...
    fastify.get('/admin/wallet/stats', async (request, reply) => {
        ...
        const [usersRes, kycRes, depositRes, withdrawRes, p2pRes, todayRes, chartRes, cgltRes, swapsTodayRes,] = await Promise.all([
    ...
    fastify.get('/admin/wallet/users', {
        schema: {
            querystring: {
                type: 'object',
                properties: {
    ...
    fastify.get('/admin/wallet/users/:id', async (request, reply) => {
        if (!requireAdmin(request.isAdmin)) {
            return reply.status(403).send({ error: 'Admin access required' });
        }
    ...
    fastify.post('/admin/wallet/users/:id/block', async (request, reply) => {
        if (!requireAdmin(request.isAdmin)) {
            return reply.status(403).send({ error: 'Admin access required' });
        }
        const { id } = request.params;
    ...
    fastify.post('/admin/wallet/users/:id/unblock', async (request, reply) => {
        if (!requireAdmin(request.isAdmin)) {
            return reply.status(403).send({ error: 'Admin access required' });
        }
        const { id } = request.params;
    ...
    fastify.post('/admin/wallet/users/:id/kyc/approve', async (request, reply) => {
        if (!requireAdmin(request.isAdmin)) {
            return reply.status(403).send({ error: 'Admin access required' });
        }
        const { id } = request.params;
    ...
    fastify.post('/admin/wallet/adjust', {
        schema: {
            body: {
                type: 'object',
        ...
        await fastify.supabase.from('ledger_entries').insert({
            ...
            reference: `admin:${(0, node_crypto_1.randomUUID)()}`,
            note: reason,
    ...
    fastify.get('/admin/wallet/kyc', {
        ...
        let q = fastify.supabase
            ...
            .select('id, wallet_user_id, status, doc_type, full_name, birth_date, doc_number, doc_front_url, doc_back_url, selfie_url, reviewer_note, submitted_at, reviewed_at, payguard_confidence, payguard_decision, wallet_users(id, phone, full_name, balance_cdf, kyc_level, is_verified)', { count: 'exact' })
    ...
    fastify.post('/admin/wallet/kyc/:id/approve', async (request, reply) => {
        if (!requireAdmin(request.isAdmin)) {
            return reply.status(403).send({ error: 'Admin access required' });
        }
        const { id } = request.params;
        ...
        if (!sub)
            return reply.status(404).send({ error: 'KYC submission not found' });
        ...
        if (submissionRes.error)
            return reply.status(500).send({ error: submissionRes.error.message });
        if (userRes.error)
            return reply.status(500).send({ error: userRes.error.message });
    ...
    fastify.post('/admin/wallet/kyc/:id/reject', {
        schema: {
            body: {
                type: 'object',
                properties: {
    ...
    fastify.get('/admin/wallet/transactions', {
        ...
        let q = fastify.supabase
            .from('transactions')
            .select('id, wallet_user_id, direction, operator, phone, amount, fee, net_amount, currency, status, reference, created_at, updated_at, swap_direction, cglt_amount, usdt_amount, blockchain_tx_hash, wallet_users(phone, full_name)', { count: 'exact' })
    ...
    fastify.get('/admin/wallet/transactions/export', async (request, reply) => {
        if (!requireAdmin(request.isAdmin)) {
            return reply.status(403).send({ error: 'Admin access required' });
        }
        const { direction, status, operator, date_from, date_to, phone } = request.query;
        let q = fastify.supabase
            .from('transactions')
    ...
    fastify.post('/admin/merchants/:id/mode', {
        schema: {
            body: {
                type: 'object',
                required: ['mode'],
    ...
    fastify.post('/admin/merchants/:id/kyc/approve', async (request, reply) => {
        if (!requireAdmin(request.isAdmin)) {
            return reply.status(403).send({ error: 'Admin access required' });
        }
        const { id } = request.params;
    ...
    fastify.get('/admin/cdp-wallets-stats', {
        schema: {
            querystring: {
                type: 'object',
                properties: {
    ...
    fastify.post('/admin/merchants/:id/kyc/reject', {
        schema: {
            body: {
                type: 'object',
...
exports.default = adminWalletRoute;
