...
const SUPPORTED_NETWORKS = ['BSC', 'ERC20', 'TRC20', 'Polygon', 'Base', 'Arbitrum'];
/* ── ERC-20 balanceOf(address) function selector ────────────────────────── */
const BALANCE_OF_SELECTOR = '0x70a08231';
/* ── Audit log helper for wallet lifecycle events ───────────────────────── */
async function auditWalletLog(supabase, action, wallet, receiptsCount) {
...
function resolveToken(asset, network) {
    if (network === 'BSC')
        return BSC_TOKENS[asset] ?? null;
    return null;
}
/* ── Fetch ERC-20 balance via JSON-RPC eth_call ─────────────────────────── */
async function fetchOnChainBalance(rpcUrl, contract, address, decimals) {
 ...
 * Plugin
 * ═══════════════════════════════════════════════════════════════════════════ */
const adminTreasuryCryptoAssetsRoute = async (fastify) => {
    /* ════════════════════════════════════════════════════════════════════
    ...
    fastify.get('/admin/treasury/crypto-assets', async (request, reply) => {
        ...
        if (walletsErr)
            return reply.status(500).send({ error: walletsErr.message });
        if (!wallets?.length)
        ...
        const results = await Promise.all(wallets.map(async (w) => {
            ...
            /* ── 3. Difference ───────────────────────────────────────── */
            const difference = onchainBalance !== null ? onchainBalance - confirmedTotal : null;
            /* ── 4. Status ───────────────────────────────────────────── */
            let status = 'unknown';
            if (difference !== null) {
                if (difference === 0)
                    ...
                    status = 'warning'; // more on-chain than accounted
                else
                    status = 'alert'; // accounting > on-chain (inconsistency)
            }
            return {
                wallet_id: w.id,
                label: w.label,
     ...
     * ════════════════════════════════════════════════════════════════════ */
    fastify.post('/admin/treasury/crypto-wallets', {
        schema: {
            body: {
                type: 'object',
     ...
     * ════════════════════════════════════════════════════════════════════ */
    fastify.delete('/admin/treasury/crypto-wallets/:id', async (request, reply) => {
        if (!request.isAdmin)
            return reply.status(403).send({ error: 'Admin access required' });
        ...
        if (usedCount === 0) {
            ...
            await auditWalletLog(fastify.supabase, 'treasury_wallet_deleted', wallet, usedCount);
            fastify.log.info({ id: wallet.id, label: wallet.label, asset: wallet.asset, network: wallet.network }, '[treasury-assets] wallet hard-deleted');
        ...
        const { error: updErr } = await fastify.supabase
            ...
            .eq('id', request.params.id);
        if (updErr)
            return reply.status(500).send({ error: updErr.message });
        await auditWalletLog(fastify.supabase, 'treasury_wallet_deactivated', wallet, usedCount);
        fastify.log.info({ id: wallet.id, label: wallet.label, receipts: usedCount }, '[treasury-assets] wallet deactivated');
     ...
     * ════════════════════════════════════════════════════════════════════ */
    fastify.patch('/admin/treasury/crypto-wallets/:id', {
        schema: {
            body: {
                type: 'object',
                additionalProperties: false,
...
exports.default = adminTreasuryCryptoAssetsRoute;
//# sourceMappingURL=treasury-crypto-assets.js.map