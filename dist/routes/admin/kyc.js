...
const adminKycRoute = async (fastify) => {
    /* ── GET /v1/admin/kyc ──────────────────────────────────── */
    ...
    fastify.post('/admin/kyc/:merchant_id/approve', async (request, reply) => {
        if (!request.isAdmin)
            return reply.status(403).send({ error: 'Admin access required', statusCode: 403 });
        ...
        (0, email_js_1.sendKycApprovedEmail)(merchant.email, merchant.name).catch((err) => {
    ...
    fastify.post('/admin/kyc/:merchant_id/reject', {
        schema: {
            body: {
                type: 'object',
                required: ['notes'],
        ...
        (0, email_js_1.sendKycRejectedEmail)(merchant.email, merchant.name, notes).catch((err) => {
