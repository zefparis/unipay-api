...
const adi_verify_1 = require("../../lib/adi-verify");
/* ── HMAC verification ──────────────────────────────────────────────────── */
...
/* ── Fastify plugin ─────────────────────────────────────────────────────── */
const adiDepositRoute = async (fastify) => {
     ...
     * ──────────────────────────────────────────────────────────────────────── */
    fastify.post('/adi/deposit-notify', {
        ...
        const secret = env_1.env.PREDICTSTREET_SERVER_SECRET;
        ...
        const receipt = await (0, adi_withdrawal_1.getAdiTransactionReceipt)(tx_hash);
        if (!receipt) {
        ...
        const verifyResult = (0, adi_verify_1.verifyAdiTransfer)({ logs: receipt.logs, status: receipt.status ?? 0 }, 'USDC', env_1.env.ADI_SETTLEMENT_ADDRESS, amount_usdc);
        if (!verifyResult.verified) {
            fastify.log.warn({ tx_hash, payout_id, reasons: verifyResult.blocking_reasons, detail: verifyResult.reason }, '[adi-deposit] on-chain verification failed');
        ...
        fastify.log.info({ payout_id, user_id, tx_hash, amount_usdc, amount_cdf }, '[adi-deposit] deposit confirmed and balance credited');
        /* ── 6. Return success ──────────────────────────────────────────── */
        return reply.send({ ok: true, credited_cdf: amount_cdf });
    });
};
exports.default = adiDepositRoute;
//# sourceMappingURL=adi-deposit.js.map