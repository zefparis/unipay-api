...
const adi_withdrawal_1 = require("../../lib/adi-withdrawal");
const avada_1 = require("../../services/avada");
/* ── HMAC helpers ───────────────────────────────────────────────────────────── */
function signHmac(payload, secret) {
    return node_crypto_1.default.createHmac('sha256', secret).update(payload, 'utf8').digest('hex');
}
function verifyHmac(rawBody, signature, secret) {
    const expected = signHmac(rawBody, secret);
    try {
        return node_crypto_1.default.timingSafeEqual(Buffer.from(signature.toLowerCase()), Buffer.from(expected.toLowerCase()));
    }
    catch {
        return false;
    }
...
const ALLOWED_OPERATORS = ['orange', 'airtel', 'afrimoney'];
/* ── Fastify plugin ─────────────────────────────────────────────────────────── */
const adiPayoutRoute = async (fastify) => {
    /* ──────────────────────────────────────────────────────────────────────────
     * A) POST /v1/adi/payout-request
     * Auth: wallet JWT
     * Body: { amount_cdf, mobile_number, operator }
     * ────────────────────────────────────────────────────────────────────────── */
    fastify.post('/adi/payout-request', {
        ...
        if (!env_1.env.PREDICTSTREET_PAYOUT_URL) {
            return reply.status(503).send({ error: 'adi_payout_not_configured' });
        }
        if (!env_1.env.PREDICTSTREET_SERVER_SECRET) {
        ...
        const withdrawalId = node_crypto_1.default.randomUUID();
        ...
        const psSignature = signHmac(psPayload, env_1.env.PREDICTSTREET_SERVER_SECRET);
        try {
            const psRes = await fetch(env_1.env.PREDICTSTREET_PAYOUT_URL, {
                method: 'POST',
                headers: {
            ...
            if (!psRes.ok) {
                ...
                fastify.log.warn({ withdrawalId, status: psRes.status, errText }, '[adi-payout] PredictStreet payout webhook returned non-2xx — withdrawal remains pending');
     ...
     * ────────────────────────────────────────────────────────────────────────── */
    fastify.post('/adi/payout-status', {
        ...
        /* 1. HMAC verification */
        const signature = (request.headers['x-predictstreet-signature'] ?? '');
        if (!signature)
            return reply.status(401).send({ error: 'missing_signature' });
        const rawBody = JSON.stringify(request.body);
        if (!verifyHmac(rawBody, signature, env_1.env.PREDICTSTREET_SERVER_SECRET)) {
        ...
        setImmediate(async () => {
            try {
                const confirmed = await (0, adi_withdrawal_1.waitForConfirmations)(tx_hash, 12);
                if (!confirmed) {
                ...
                let avadaId;
                try {
                    const result = await (0, avada_1.initiatePayout)(wr.operator, wr.mobile_number, Number(wr.amount_cdf), avadaRef, 'CDF');
                ...
                await fastify.supabase
                    ...
                    .eq('id', payout_id);
                fastify.log.info({ payout_id, tx_hash, avadaId, amount_cdf: wr.amount_cdf, mobile: wr.mobile_number }, '[adi-payout-status] CDF payout triggered via Avada');
            }
            catch (err) {
...
exports.default = adiPayoutRoute;
//# sourceMappingURL=adi-payout.js.map