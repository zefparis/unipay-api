...
async function fetchUsdtCdfRate(fallback) {
    if (rateCache.rate > 0 && Date.now() - rateCache.ts < RATE_TTL)
...
/* ── Fastify plugin ─────────────────────────────────────────────────────── */
const walletTransakRoute = async (fastify) => {
    ...
    fastify.get('/rates/usdt-cdf', async (_req, reply) => {
        const fallback = Number(env_1.env.FIAT_USD_CDF_RATE ?? '2850');
     ...
     * ───────────────────────────────────────────────────────────────────────── */
    fastify.post('/wallet/transak/init', {
        ...
        const successRedirect = redirect_url ?? `${appUrl}/wallet/deposit?transak_done=${orderId}`;
        ...
        fastify.log.info({ orderId, walletId: wallet.id, amount_fiat, currency, isCustody }, '[transak] order created');
    ...
    fastify.register(async (sub) => {
        ...
        sub.post('/wallet/transak/webhook', async (request, reply) => {
            ...
            const sig = request.headers['x-transak-signature'];
            if (!sig)
            ...
            fastify.log.info({ eventID, partnerOrderId, status: newStatus }, '[transak] webhook event');
            ...
            if (newStatus === 'COMPLETED' &&
                ...
                if (cryptoAmt > 0) {
                    ...
                    if (creditErr) {
                        fastify.log.error({ err: creditErr, userId: order.user_id, cryptoAmt }, '[transak] wallet_credit_usd failed');
                    }
                    else {
                        fastify.log.info({ userId: order.user_id, cryptoAmt }, '[transak] usd_balance credited');
     ...
     * ───────────────────────────────────────────────────────────────────────── */
    fastify.get('/wallet/transak/orders/:orderId', async (request, reply) => {
        if (!env_1.env.JWT_SECRET)
            return reply.status(500).send({ error: 'auth_not_configured' });
        const payload = (0, wallet_jwt_1.requireWallet)(request.headers.authorization, env_1.env.JWT_SECRET);
        if (!payload)
...
exports.default = walletTransakRoute;
