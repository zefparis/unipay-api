...
const SUPPORTED_NETWORKS = ['BSC'];
/* ── Network fee (USDT) ───────────────────────────────────────────────── */
const NETWORK_FEE = { BSC: 0.5 };
/* ── Address regex (BSC = EVM) ─────────────────────────────────────────── */
const EVM_ADDR = /^0x[a-fA-F0-9]{40}$/;
/* ── Minimum net withdrawal (USDT) ────────────────────────────────────── */
const MIN_NET = 5;
const walletCryptoWithdrawRoute = async (fastify) => {
    /* ── POST /v1/wallet/crypto-withdraw ─────────────────────────────────── */
    fastify.post('/wallet/crypto-withdraw', {
        schema: {
            body: {
                type: 'object',
        ...
        fastify.log.info({ withdrawalId, walletId, network, amount, fee, netAmount, addrMasked }, 'USDT withdrawal — sending on-chain (BSC hot wallet)');
        /* ── 6. Send on-chain via hot wallet ─────────────────────────────── */
        try {
            const { txHash } = await (0, bsc_withdrawal_1.sendUsdt)({
            ...
            await fastify.supabase
                ...
                .eq('id', withdrawalId);
            fastify.log.info({ withdrawalId, txHash, walletId, network, netAmount, addrMasked }, 'USDT withdrawal sent on-chain (BSC hot wallet)');
            ...
            const reason = err?.message ?? 'On-chain error';
    ...
    /* ── GET /v1/wallet/crypto-withdrawals ───────────────────────────────── */
    fastify.get('/wallet/crypto-withdrawals', {
        schema: {
            querystring: {
                type: 'object',
        ...
        const { data, error, count } = await fastify.supabase
            .from('withdrawal_requests')
            .select('id, amount, network, destination_address, fee, status, binance_withdraw_id, tx_hash, failure_reason, created_at, updated_at', { count: 'exact' })
...
exports.default = walletCryptoWithdrawRoute;
//# sourceMappingURL=crypto-withdraw.js.map