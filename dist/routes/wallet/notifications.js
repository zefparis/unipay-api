...
const walletNotificationsRoute = async (fastify) => {
    /* ── GET /v1/wallet/push/vapid-public-key — public ────────────── */
    fastify.get('/wallet/push/vapid-public-key', async (_request, reply) => {
        return reply.send({ publicKey: env_1.env.VAPID_PUBLIC_KEY ?? null });
    });
    /* ── POST /v1/wallet/push/subscribe ────────────────────────────── */
    fastify.post('/wallet/push/subscribe', {
        schema: {
            body: {
                type: 'object',
        ...
        await fastify.supabase
            .from('push_subscriptions')
            .upsert({ user_id: userId, endpoint, p256dh: keys.p256dh, auth: keys.auth, user_agent: userAgent ?? null }, { onConflict: 'endpoint' });
        return reply.send({ ok: true });
    });
    /* ── DELETE /v1/wallet/push/unsubscribe ────────────────────────── */
    fastify.delete('/wallet/push/unsubscribe', {
        schema: {
            body: {
                type: 'object',
                required: ['endpoint'],
    ...
    fastify.get('/wallet/notifications', {
        schema: {
            querystring: {
                type: 'object',
                properties: {
    ...
    fastify.get('/wallet/notifications/unread-count', async (request, reply) => {
        const walletPayload = (0, wallet_jwt_1.requireWallet)(request.headers.authorization, env_1.env.JWT_SECRET);
        if (!walletPayload)
            return reply.status(401).send({ error: 'Unauthorized' });
        const { count } = await fastify.supabase
    ...
    fastify.post('/wallet/notifications/read', {
        schema: {
            body: {
                type: 'object',
        ...
        const query = fastify.supabase
            ...
            .eq('user_id', userId);
        if (request.body?.id)
            query.eq('id', request.body.id);
        await query;
        return reply.send({ ok: true });
    });
    /* ── GET /v1/wallet/pref/notifications ──────────────────────────── */
    fastify.get('/wallet/pref/notifications', async (request, reply) => {
        const walletPayload = (0, wallet_jwt_1.requireWallet)(request.headers.authorization, env_1.env.JWT_SECRET);
        if (!walletPayload)
            return reply.status(401).send({ error: 'Unauthorized' });
    ...
    fastify.patch('/wallet/pref/notifications', {
        schema: {
            body: {
                type: 'object',
                properties: {
...
exports.default = walletNotificationsRoute;
