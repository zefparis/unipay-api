...
function requireGamingKey(request, reply) {
    const expected = env_1.env.GAMING_API_KEY;
    if (!expected) {
...
const cgltGamingRoute = async (fastify) => {
    /* ── POST /v1/wallet/cglt-debit — place a CGLT bet ─────────── */
    fastify.post('/wallet/cglt-debit', {
        schema: {
            body: {
                type: 'object',
        ...
        if (!requireGamingKey(request, reply))
            return;
    ...
    fastify.post('/wallet/cglt-credit', {
        schema: {
            body: {
                type: 'object',
        ...
        if (wallet.blockchain_address) {
            try {
                blockchainTxHash = await (0, blockchain_1.mintCGLT)(wallet.blockchain_address, amount, tx_ref);
            }
            catch (err) {
    ...
    fastify.get('/wallet/cglt-balance', {
        schema: {
            querystring: {
                type: 'object',
                required: ['phone'],
        ...
        const cgltBalance = Number(wallet.cglt_balance ?? 0);
        let equivalentUsdt = null;
        try {
            const { rate } = await (0, blockchain_1.getSwapRate)();
            if (rate > 0)
                equivalentUsdt = cgltBalance / rate;
        }
        catch (err) {
    ...
    fastify.post('/wallet/cglt-withdraw-bsc', {
        schema: {
            body: {
                type: 'object',
        ...
        let bscTxHash = null;
        try {
            bscTxHash = await (0, blockchain_1.mintWCGLTonBSC)(bsc_address, wCGLTAmount);
        }
        catch (err) {
    ...
    fastify.post('/wallet/user/cglt-withdraw-bsc', {
        schema: {
            body: {
                type: 'object',
        ...
        const jwtPayload = (0, wallet_jwt_1.requireWallet)(request.headers.authorization, env_1.env.JWT_SECRET);
        if (!jwtPayload)
...
exports.default = cgltGamingRoute;
//# sourceMappingURL=cglt-gaming.js.map