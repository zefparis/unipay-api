...
-- so we DROP IF EXISTS first to make the migration idempotent.
DROP POLICY IF EXISTS support_conversations_wallet_user_isolation
  ON public.support_conversations;

CREATE POLICY support_conversations_wallet_user_isolation
  ON public.support_conversations
  FOR ALL
...
CREATE POLICY support_messages_via_wallet_conversation
  ON public.support_messages
  FOR ALL
  USING (
    EXISTS (
